...
from bor_solr_importer.bor_api.services.bor_solr.doc_models import Address, DateRange, Entity, EntityRole
from bor_solr_importer.bor_api.utils.data_converters import get_btr_owner
...
def get_party_name(item_dict: dict[str, str]) -> str:
    ...
    if item_dict.get("organization_name"):
...
def get_address(item_dict: dict[str, str], is_party_address: bool, is_mail: bool) -> Address:
...
def get_entity_role(item_dict: dict[str, str], party_id: str, business: Entity) -> EntityRole:
    ...
    if cessation_date := item_dict.get("cessation_date"):
...
def set_party_entity(item_dict: dict[str, str],
    ...
    if party_already_added:
        ...
        use_mailing = not item_dict.get("delivery_addr_id") and source == "COLIN"
    ...
    return prepped_data.get(party_id)
...
def update_party_links(prepped_data: dict[str, Entity],  # noqa: PLR0913
    ...
    if corp_num in child_link:
        ...
        if prev_id in child_link[corp_num]:
            ...
            # TODO: test this
            prepped_data.pop(dupe_parent_id, None)
...
def get_entities(prepped_data: dict[str, Entity]) -> list[dict]:
    ...
    entities = []
    for entity in prepped_data.values():
    ...
    # current_app.logger.debug(f"entities with missing appointment date: {len(missing_data['appointment_date'])}")  # noqa: ERA001
    return entities
...
def prep_data(data: list[dict[str, str]],  # pylint: disable=too-many-locals,too-many-branches; Due to debug logging
    ...
    for item in data:
        ...
        if party_entity and item_dict.get("prev_party_id"):
            ...
            if item_dict.get("event_typ_cd") in ["CONVICORP", "CONVAMAL", "CONVCIN"]:
                ...
                continue
            if filing_date := item_dict.get("effective_dt") or item_dict.get("event_timestmp"):
